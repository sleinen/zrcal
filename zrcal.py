...
class GetCal(webapp2.RequestHandler):
    def get(self, zip=None, types=None):
        ...
        cal.add('version', '2.0')
        cal_add_name(cal, 'Entsorgungskalender %d' % (zip), self.request)
        cal_add_desc(cal, ('Entsorgungskalender for PLZ %d.  '
                           +'Generated by http://zrcal.leinen.ch/ '
                           +'based on Open Government Data '
                           +'provided by the City of Zurich') \
                     % (zip), self.request)
